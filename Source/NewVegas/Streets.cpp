...
Streets::Streets()
{
    ...
    texture[0]->Load2D((RESOURCE_PATH::TEXTURES + "blvd2.jpg").c_str(), GL_REPEAT);

    texture[1] = new Texture2D();
    texture[1]->Load2D((RESOURCE_PATH::TEXTURES + "street1.jpg").c_str(), GL_REPEAT);
    ...
    while (lastx <= UIConstants::Map::MAX) {
        lastx = lastx + (rand() % 5 + 1) * UIConstants::Streets::MIN_DIST;
    ...
    while (lastz <= UIConstants::Map::MAX) {
        lastz = lastz + (rand() % 3 + 1) * UIConstants::Streets::MIN_DIST;
